...
RULES_TEXT = (
    "📖 Правила игры «Киллер»\n\n"
    "1. Игра построена на принципе честной игры. НАРУШЕНИЕ ПРИВОДИТ К БАНУ.\n"
    "2. Каждый участник одновременно охотник и жертва.\n"
    "3. Игра стартует одновременно, когда каждый получает досье на жертву через бота "
    ...
    "При свидетелях убийство не засчитывается. Убийство в библиотеках и коворках при "
    "стеклянных пространствах считаются как открытые.\n"
...
REG_STEPS = [
    ...
    ("dormitory", "Живёшь в общаге? Если да — укажи корпус. Если нет — район проживания:", False),
    ("habits", "Дополнительно: укажи что-то, что хочешь рассказать о себе, например, с кем общаешься.\n"
               "Можно пропустить:", True),
    ("photo_id", "Загрузи своё фото.", False),
]
...
async def ask_reg_step(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if key == "photo_id":
        await update.message.reply_text(question, reply_markup=K.registration_nav())
    else:
        await update.message.reply_text(question, reply_markup=K.registration_nav(skippable))
    return REG_FLOW
